...
function install_riak
{
	...
	rsync -avz "riak-1.2.0" $download_dir
	(cd "$download_dir/riak-1.2.0" && make rel)
}
...
function start_riak_and_create_cluster
{
	...
	do
		(cd "$download_dir/riak-1.2.0/dev" && $node/bin/riak start)
	done

	#making cluster
	for node in dev2 dev3 dev4
	do
		(cd "$download_dir/riak-1.2.0/dev" && $node/bin/riak-admin join "dev1@127.0.0.1")
	done
}
